...
LONG WINAPI _WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (self && hWnd == self->hWnd) switch (uMsg) {
        ...
        case WM_CHAR:
            _event(self, key_pressed)(self, wParam);
            return 0;

        case WM_CLOSE:
            _event(self, closing)(self);
            return 0;
    }
...
bool frame_create(Frame* self, char const* title) {
    ...
    self->hWnd = CreateWindow("OpenGLFrame", title,
            ...
            NULL, NULL/*:menu*/, hInstance, NULL);

    PIXELFORMATDESCRIPTOR pfd = {
        .nSize= sizeof pfd,
        .nVersion= 1,
        .dwFlags= PFD_DRAW_TO_WINDOW | PFD_SUPPORT_OPENGL, // | PFD_DOUBLEBUFFER; // probably
