...
void chans(buf const filepath, buf const channels, buf const outfile) {
...
void chunk(buf const filepath, buf const chunk, buf const outfile) {
    buf const pngbuf = file_read(filepath);
    png_data const png = png_data_parse(pngbuf);

    if (!strcmp("-l", (char*)chunk.ptr)) {
        buf const res = png_chunknames(&png);
        file_write(outfile, res);
    }
        ...
        buf const res = png_find_chunk(&png, png_chunk_type_str(chunk.ptr));
        file_write(outfile, res);
    }
}
